        ...
        <div class="bg-white rounded-lg overflow-hidden shadow-lg">
            <div class="relative">
                    @if($media && !empty($media->chemin))
                        <img src="{{ asset($media->chemin) }}" alt="{{ $contenu->titre }}" class="w-full h-48 object-cover">
                @else
                ...
                <div class="absolute left-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent w-full p-4">
                        <h3 class="text-white text-lg sm:text-xl font-extrabold">{{ $contenu->titre }}</h3>
                </div>
            </div>
            ...
            <div class="p-6">
                    ...
                    <p class="text-gray-700 mb-4 leading-relaxed text-sm sm:text-base">{{ \Illuminate\Support\Str::limit($contenu->description ?? strip_tags($contenu->contenu_html), 160) }}</p>

                <div class="flex items-center justify-between">
                        <a href="{{ route('front.contenu.pay', $contenu->id) }}" class="px-4 py-2 bg-green-600 text-white rounded-md shadow">Payer / Lire</a>
